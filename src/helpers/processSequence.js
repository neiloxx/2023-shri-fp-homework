...
const getBinaryNumber = compose(api.get(GET_NUMBERS_BASE),  assoc('number', __, NUMBER_PARAMS));
...
const isValueValid = allPass([isLengthsMoreThenTwo, isLengthLowerThenTen, isSymbolsCorrect])
...
const processSequence = ({ value, writeLog, handleSuccess, handleError }) => {
  ...
  const runSequence = ifElse(isValueValid, sequence, partial(handleError, ['ValidationError']));
  const runBeforeCheck = compose(
    runSequence,
    tapLog,
  );
